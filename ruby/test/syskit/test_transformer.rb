...
describe Transformer do
    describe "the handling of devices" do
        ...
        it "detects unknown frames" do
            ...
            req = task_m.use_frames("from" => "unknown")

            capture_log(Transformer, :fatal) do
                assert_raises(Transformer::InvalidConfiguration) do
        ...
        it "detects conflicts between the frame selection on the task and on the device" do
            ...
            req = task_m.use_frames("from" => "other_from")
                        .with_arguments(test_dev: dev)
                        .transformer { frames "test_from", "test_to", "other_from"}
        ...
        it "links the task and device frames through transform ports" do
            ...
            req = task_m.with_arguments(test_dev: dev)
                        .transformer { frames "test_from", "test_to" }
            task = syskit_stub_and_deploy(req)
        ...
        it "links the task and device frames through data ports" do
            ...
            req = task_m.with_arguments(test_dev: dev)
                        .transformer { frames "test" }
            task = syskit_stub_and_deploy(req)
            assert_equal Hash["frame" => "test"], task.selected_frames
        end
    end
end
